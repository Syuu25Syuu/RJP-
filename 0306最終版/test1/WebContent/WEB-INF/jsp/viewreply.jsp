...
<script>
	$(function(){
		var null_check = $(".null_check").val();
		if(null_check == "このツイートは削除されました"){
			$(".tweetdata").eq(0).text("このツイートは削除されました");
		}
 	...
 	<c:forEach var="data" items="${requestbeen}" >
		<div class="tweetdata" id = "tweetdata${data.tweetId}" >
		 	...
		 	<form method = 'post' id="myFORM" action = 'showprofiles'>
		 		...
		 		<input type="hidden" name = "user_session" type = "text" value="${sessionScope.token.sessionToken}">
		 		<input type="hidden" class="null_check" name = "user_id" type = "text" value="${data.serialuserid}">
