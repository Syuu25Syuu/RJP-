...
<body>
	...
	<table border="0">
		<c:forEach var = "search" items = "${requestbeen}" >
				<tr>
					<td>
						<img src = "${search.userIcon}" style = "width:30px; height:30px;" class="pro">
					</td>
					<td style="font-size:20px;">${search.userName}</td>
					<td id="userid" class="userid">@${search.userId}</td>
					<td>
						<form action="directmessage" method="post">
							<input type="hidden" id="" name="followed_no" class="${search.userNo}"value="${search.userNo}">
							<input type="hidden" class="check_serialno"value="${search.userNo}">
							...
							<input type="submit" id="${search.userNo}"  class="followbtn" value="チャットする">
						</form>
					</td>
				</tr>
		</c:forEach>
