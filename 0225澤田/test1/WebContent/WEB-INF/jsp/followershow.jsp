	...
	<table border="1">
		...
		<c:forEach var = "search" items = "${result}" >
			<form action="directmessage" method="post">
				<tr><td><img src="${search.userIcon}" style="height:30px; width:30px;"></td><td id="userid">${search.userId}</td><td>${search.userName}</td><td><input type="button" id="" name="followed_no" class="${search.userNo}"value="${search.userNo}">
